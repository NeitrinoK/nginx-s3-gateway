...
# Focus: JSON key aliases (AccessKeyId/SecretAccessKey/Token) match njs behavior.
...
http {
    ...
    server {
        ...
        location / {
            ...
            set $instance_credential_json '{"AccessKeyId":"UPPERAK","SecretAccessKey":"UPPERSECRET","Token":"TOKEN-ALIAS","Expiration":"2030-01-01T00:00:00Z"}';
...
like($resp, qr/\r\nX-TOKEN: \[TOKEN-ALIAS\]\r\n/i,
    '$awsSessionToken reads Token alias from credentials JSON');
like($resp, qr/\r\nX-S3AUTH: AWS4-HMAC-SHA256 Credential=UPPERAK\//i,
    'SigV4 auth uses AccessKeyId alias from credentials JSON');
like($resp, qr/SignedHeaders=host;x-amz-content-sha256;x-amz-date;x-amz-security-token/i,
    'SigV4 signed headers include security token for Token alias');
