...
# Focus: HEAD + Range for file uses sliced branch, directory still keeps pre-listing priority.
...
my $file_head = http(<<'EOF_REQ');
HEAD /video.bin HTTP/1.0
Host: localhost
Range: bytes=0-10

EOF_REQ

my $dir_head = http(<<'EOF_REQ');
HEAD /dir/ HTTP/1.0
Host: localhost
Range: bytes=0-10
...
like($get_range, qr/^HTTP\/1\.1 204 /m,
    'GET file request with Range still routes to @s3 when PROVIDE_INDEX_PAGE is enabled');
like($file_head, qr/^HTTP\/1\.1 204 /m,
    'HEAD file request ignores slicing and routes to @s3');
like($dir_head, qr/^HTTP\/1\.1 200 /m,
    'HEAD directory request with Range still prefers @s3PreListing');
