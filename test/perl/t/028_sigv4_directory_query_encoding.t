...
# Focus: directory listing prefix is percent-decoded then re-encoded canonically.
...
like($encoded, qr/\r\nX-S3URI: \/bucket-1\?delimiter=%2F&prefix=a%20b%2Fc%2Bd%2F\r\n/i,
    'encoded input normalizes to canonical query prefix');
like($plain, qr/\r\nX-S3URI: \/bucket-1\?delimiter=%2F&prefix=a%20b%2Fc%2Bd%2F\r\n/i,
    'plain input produces same canonical query prefix');

my ($enc_auth) = ($encoded =~ /\r\nX-S3AUTH:\s*([^\r\n]+)\r\n/i);
my ($plain_auth) = ($plain =~ /\r\nX-S3AUTH:\s*([^\r\n]+)\r\n/i);
is($enc_auth, $plain_auth,
   'normalized canonical query yields identical SigV4 signatures');
