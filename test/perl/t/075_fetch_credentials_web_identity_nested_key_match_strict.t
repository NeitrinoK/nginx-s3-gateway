...
# Focus: nested STS key lookup must match JSON object keys only,
# not string values containing the same key text.
...
my $token_file = "/tmp/nginx-s3gw-web-token-keymatch-$$";
open my $tfh, '>', $token_file or die "open $token_file: $!";
print {$tfh} "dummy-token";
close $tfh;

END {
    unlink $token_file if defined $token_file && -f $token_file;
}
...
http {
    ...
    server {
        ...
        location = /sts {
            ...
            return 200 '{"note":"AssumeRoleWithWebIdentityResponse","trap":{"AssumeRoleWithWebIdentityResult":{"Credentials":{"AccessKeyId":"TRAP_AK","SecretAccessKey":"TRAP_SK","SessionToken":"TRAP_ST","Expiration":"2099-01-02T03:04:05Z"}}},"AssumeRoleWithWebIdentityResponse":{"AssumeRoleWithWebIdentityResult":{"Credentials":{"AccessKeyId":"REAL_AK","SecretAccessKey":"REAL_SK","SessionToken":"REAL_ST","Expiration":"2099-02-03T04:05:06Z"}}}}';
        }
    }
}
EOF_CONF
...
like($resp, qr/^HTTP\/1\.1 200 /m,
    'web-identity refresh succeeds');
like($resp, qr/\r\nContent-Length: 0\r\n/i,
    'fetchCredentials returns empty body status response');
like($resp, qr/\r\nContent-Type: text\/plain\r\n/i,
    'fetchCredentials response content-type is text/plain');
like($resp, qr/\r\nX-CACHED: \{"accessKeyId":"REAL_AK","secretAccessKey":"REAL_SK","sessionToken":"REAL_ST","expiration":"2099-02-03T04:05:06Z"\}\r\n/i,
    'cached credentials come from real nested response object');
unlike($resp, qr/TRAP_AK/i,
    'trap object after string-value decoy is ignored');
unlike($resp, qr/OLD_AK/i,
