...
# Focus: virtual-style listing prefix canonicalization handles encoded and plain forms equally.
...
my $encoded = http_get('/encoded');
my $plain = http_get('/plain');

like($encoded, qr/^HTTP\/1\.1 200 /m, 'encoded directory endpoint returns 200');
like($plain, qr/^HTTP\/1\.1 200 /m, 'plain directory endpoint returns 200');
like($encoded, qr/\r\nX-S3URI: \?delimiter=%2F&prefix=a%2Fb%2Fc%20d%2Be%2F\r\n/i,
    'encoded virtual-style URI normalizes to canonical listing prefix');
like($plain, qr/\r\nX-S3URI: \?delimiter=%2F&prefix=a%2Fb%2Fc%20d%2Be%2F\r\n/i,
    'plain virtual-style URI produces the same canonical listing prefix');

my ($enc_auth) = ($encoded =~ /\r\nX-S3AUTH:\s*([^\r\n]+)\r\n/i);
my ($plain_auth) = ($plain =~ /\r\nX-S3AUTH:\s*([^\r\n]+)\r\n/i);
is($enc_auth, $plain_auth,
   'identical canonical listing query in virtual style yields identical SigV4 signature');
like($enc_auth, qr/^AWS4-HMAC-SHA256 Credential=AKIDEXAMPLE\//,
    'resulting virtual-style signature remains valid SigV4 auth header');
