...
# Focus: malformed cached payload should be ignored and recomputed (njs parity).
...
http {
    ...
    server {
        ...
        location /bad-cache {
            ...
            set $signing_key_hash "__TODAY__:garbage-cache";
...
my $resp = http_get('/bad-cache');
like($resp, qr/^HTTP\/1\.1 200 /m, 'request returns 200 with malformed cache payload');
like($resp, qr/\r\nX-S3AUTH: AWS4-HMAC-SHA256 Credential=AKIDEXAMPLE\//i,
    'SigV4 auth is generated even when cache payload is malformed');
unlike($resp, qr/\r\nX-SIGNING-KEY-HASH:\s*\Q$today\E:garbage-cache\r\n/i,
    'malformed cached value is not reused as-is');
like($resp, qr/\r\nX-SIGNING-KEY-HASH:\s*\Q$today\E:\{"type":"Buffer","data":\[(?:\d{1,3},){31}\d{1,3}\]\}\r\n/i,
    'cache value is rebuilt in njs-compatible JSON Buffer format');

my $resp2 = http_get('/bad-cache');
like($resp2, qr/^HTTP\/1\.1 200 /m, 'repeated request remains healthy after cache rebuild');
