...
# Focus: expired file-backed credentials trigger refresh path and return 500 on refresh failure.
...
my $creds_file = "/tmp/nginx-s3gw-creds-051-$$.json";
open my $fh, '>', $creds_file or die "open $creds_file: $!";
print {$fh} '{"accessKeyId":"A","secretAccessKey":"B","sessionToken":"C","expiration":"1"}';
close $fh;
$ENV{AWS_CREDENTIALS_TEMP_FILE} = $creds_file;

END {
    unlink $creds_file if defined $creds_file && -f $creds_file;
}
...
like($resp, qr/^HTTP\/1\.1 500 /m,
    'expired file credentials trigger refresh and fail with 500');
like($resp, qr/\r\nContent-Length: 0\r\n/i,
