...
# Focus: keyval JSON credentials are used for signing and session token headers.
...
http {
    ...
    server {
        ...
        location / {
            ...
            set $instance_credential_json '{"accessKeyId":"KEYVALAK","secretAccessKey":"KEYVALSECRET","sessionToken":"TOKEN-XYZ","expiration":"2030-01-01T00:00:00Z"}';
...
like($resp, qr/\r\nX-TOKEN: \[TOKEN-XYZ\]\r\n/i,
    '$awsSessionToken exposes keyval session token');
like($resp, qr/\r\nX-S3AUTH: AWS4-HMAC-SHA256 Credential=KEYVALAK\//i,
    'SigV4 auth uses keyval access key');
like($resp, qr/SignedHeaders=host;x-amz-content-sha256;x-amz-date;x-amz-security-token/i,
    'SigV4 SignedHeaders include x-amz-security-token for keyval session token');
like($resp, qr/Signature=[0-9a-f]{64}\r\n/i,
    'SigV4 signature is produced with expected hex shape');
