...
# Focus: empty Token alias is treated as no session token.
...
http {
    ...
    server {
        ...
        location / {
            ...
            set $instance_credential_json '{"AccessKeyId":"UPPERAK","SecretAccessKey":"UPPERSECRET","Token":"","Expiration":"2030-01-01T00:00:00Z"}';
...
like($resp, qr/\r\nX-TOKEN: \[\]\r\n/i,
    '$awsSessionToken is empty for empty Token alias');
like($resp, qr/\r\nX-S3AUTH: AWS4-HMAC-SHA256 Credential=UPPERAK\//i,
    'SigV4 auth uses AccessKeyId alias');
like($resp, qr/SignedHeaders=host;x-amz-content-sha256;x-amz-date,Signature=/i,
    'SignedHeaders exclude x-amz-security-token for empty Token alias');
