...
# Focus: file-backed credentials with far-future epoch expiration skip refresh.
...
my $creds_file = "/tmp/nginx-s3gw-creds-048-$$.json";
open my $fh, '>', $creds_file or die "open $creds_file: $!";
print {$fh} '{"accessKeyId":"A","secretAccessKey":"B","sessionToken":"C","expiration":"4102444800"}';
close $fh;
...
like($resp, qr/^HTTP\/1\.1 200 /m,
    'future epoch file credentials short-circuit refresh with 200');
like($resp, qr/\r\nContent-Length: 0\r\n/i,
