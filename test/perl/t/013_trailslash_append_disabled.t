...
# Focus: APPEND_SLASH_FOR_POSSIBLE_DIRECTORY=false should never redirect.
...
like(http_get('/docs'), qr/^HTTP\/1\.1 404 /m, 'no redirect when append-slash feature is disabled');
like(http_get('/docs/'), qr/^HTTP\/1\.1 404 /m, 'directory path still returns 404');
like(http_get('/file.txt'), qr/^HTTP\/1\.1 404 /m, 'file path returns 404');
unlike(http_get('/docs'), qr/^HTTP\/1\.1 302 /m, '@trailslash is not used');
