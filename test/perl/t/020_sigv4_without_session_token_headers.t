...
# Focus: when AWS_SESSION_TOKEN is absent, SignedHeaders must not include
# x-amz-security-token and $awsSessionToken should be empty.
...
like($resp, qr/\r\nX-TOKEN: \[\]\r\n/i,
    '$awsSessionToken is empty when no session token is configured');
like($resp, qr/SignedHeaders=host;x-amz-content-sha256;x-amz-date,Signature=/i,
    'SignedHeaders exclude x-amz-security-token without session token');
unlike($resp, qr/x-amz-security-token/i,
    'SigV4 auth header does not mention x-amz-security-token');
