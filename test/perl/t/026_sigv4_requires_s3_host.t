...
# Focus: missing $s3_host still produces a signature (njs parity), but changing
# host value changes canonical request and therefore signature.
...
my $missing = http_get('/missing-host');
my $present = http_get('/with-host');

like($missing, qr/^HTTP\/1\.1 200 /m, 'missing-host endpoint returns 200');
like($present, qr/^HTTP\/1\.1 200 /m, 'with-host endpoint returns 200');
my ($missing_auth) = ($missing =~ /\r\nX-S3AUTH: \[([^\r\n]*)\]\r\n/i);
my ($present_auth) = ($present =~ /\r\nX-S3AUTH: \[([^\r\n]*)\]\r\n/i);
like($missing_auth, qr/^AWS4-HMAC-SHA256 Credential=AKIDEXAMPLE\//,
    'without $s3_host, SigV4 auth is still generated (njs parity)');
like($present_auth, qr/^AWS4-HMAC-SHA256 Credential=AKIDEXAMPLE\//,
    'with $s3_host, SigV4 auth is generated');
isnt($missing_auth, $present_auth,
     'explicit $s3_host changes canonical host and resulting signature');
