...
# Focus: object URI escaping preserves double slashes and normalizes encoded bytes.
...
http {
    ...
    server {
        ...
        location / {
            set $uri_path "/foo//bar/%7Ephoto 1.png";
...
my $resp = http_get('/anything');

like($resp, qr/^HTTP\/1\.1 200 /m, 'request returns 200');
like($resp, qr/\r\nX-S3URI: \/bucket-1\/foo\/\/bar\/~photo%201\.png\r\n/i,
    'double slashes are preserved and path is canonically escaped');
