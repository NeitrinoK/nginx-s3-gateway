...
# Focus: in virtual style, forIndexPage=true on encoded directory matches explicit encoded index object signature.
...
http {
    ...
    server {
        ...
        location /explicit-index {
            set $uri_path "/a%20b/c%2Bd/index.html";
...
like($dir, qr/^HTTP\/1\.1 200 /m, 'forIndex endpoint returns 200');
like($idx, qr/^HTTP\/1\.1 200 /m, 'explicit index endpoint returns 200');
like($dir, qr/\r\nX-S3URI: \/a%20b\/c%2Bd\/index\.html\r\n/i,
    'forIndex encoded directory resolves to canonical encoded index URI');
like($idx, qr/\r\nX-S3URI: \/a%20b\/c%2Bd\/index\.html\r\n/i,
    'explicit index URI matches canonical encoded index path');

my ($dir_auth) = ($dir =~ /\r\nX-S3AUTH:\s*([^\r\n]+)\r\n/i);
my ($idx_auth) = ($idx =~ /\r\nX-S3AUTH:\s*([^\r\n]+)\r\n/i);
is($dir_auth, $idx_auth,
   'forIndex and explicit index produce identical SigV2 signature for same canonical target');
like($dir_auth, qr/^AWS AKIDEXAMPLE:/,
    'generated auth is valid SigV2 header');
