...
# SigV4 signing tests for ngx_http_s3_gateway_c_module.
# Focus: canonical method contributes to signature; GET and HEAD differ for same object.
...
my $get_resp = http_get('/obj');
my $head_resp = http(<<'EOF_REQ');
HEAD /obj HTTP/1.0
Host: localhost

EOF_REQ

like($get_resp, qr/^HTTP\/1\.1 200 /m, 'GET endpoint returns 200');
like($head_resp, qr/^HTTP\/1\.1 200 /m, 'HEAD endpoint returns 200');

my ($get_auth) = ($get_resp =~ /\r\nX-S3AUTH:\s*([^\r\n]+)\r\n/i);
my ($head_auth) = ($head_resp =~ /\r\nX-S3AUTH:\s*([^\r\n]+)\r\n/i);

like($get_auth, qr/^AWS4-HMAC-SHA256 Credential=AKIDEXAMPLE\//,
    'GET returns a valid SigV4 auth header');
like($head_auth, qr/^AWS4-HMAC-SHA256 Credential=AKIDEXAMPLE\//,
    'HEAD returns a valid SigV4 auth header');
isnt($get_auth, $head_auth,
    'GET and HEAD signatures differ for the same object due to canonical method');
