...
# Credential-source priority tests for ngx_http_s3_gateway_c_module.
# Focus: static environment credentials take priority over keyval credentials.
...
$ENV{AWS_ACCESS_KEY_ID} = 'ENVAK';
$ENV{AWS_SECRET_ACCESS_KEY} = 'ENVSECRET';
$ENV{AWS_SESSION_TOKEN} = 'ENVTOKEN';
...
http {
    ...
    server {
        ...
        location / {
            ...
            set $instance_credential_json '{"accessKeyId":"KEYVALAK","secretAccessKey":"KEYVALSECRET","sessionToken":"KEYVALTOKEN","expiration":"2030-01-01T00:00:00Z"}';
...
like($resp, qr/\r\nX-TOKEN: \[ENVTOKEN\]\r\n/i,
    '$awsSessionToken comes from environment credentials');
like($resp, qr/\r\nX-S3AUTH: AWS4-HMAC-SHA256 Credential=ENVAK\//i,
    'SigV4 auth uses environment access key, not keyval key');
like($resp, qr/SignedHeaders=host;x-amz-content-sha256;x-amz-date;x-amz-security-token/i,
    'SignedHeaders include security-token due env session token');
unlike($resp, qr/Credential=KEYVALAK\//i,
      'keyval access key is not used when env credentials are present');
