...
# Focus: AWS_STS_REGIONAL_ENDPOINTS=regional requires AWS_REGION for web-identity refresh.
...
my $token_file = "/tmp/nginx-s3gw-web-token-missing-region-$$";
...
http {
    ...
    server {
        ...
        location /fetch {
            ...
            set $instance_credential_json '{"accessKeyId":"A","secretAccessKey":"B","sessionToken":"C","expiration":"1"}';
...
like($resp, qr/^HTTP\/1\.1 500 /m,
    'missing AWS_REGION with regional STS endpoints causes refresh failure');
like($resp, qr/\r\nContent-Length: 0\r\n/i,
    'error response keeps empty body');
