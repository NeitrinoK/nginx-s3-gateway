...
# Focus: HEAD directory detection uses percent-decoding and invalid encoding fails request.
...
my $get_resp = http_get('/headers-get');
my $head_resp = http(<<'EOF_REQ');
HEAD /headers-head-bad HTTP/1.0
Host: localhost

EOF_REQ

like($get_resp, qr/^HTTP\/1\.1 200 /m, 'sanity GET request still succeeds');
like($get_resp, qr/\r\nX-Keep: yes\r\n/i, 'non-amz header is preserved on GET path');
unlike($get_resp, qr/\r\nX-Amz-Request-Id:/i, 'x-amz header is stripped on GET path');
like($head_resp, qr/500 Internal Server Error/i,
    'HEAD path with invalid percent-encoding fails during directory decode');
