...
# Focus: invalid file-backed credentials JSON returns 500.
...
my $creds_file = "/tmp/nginx-s3gw-creds-050-$$.json";
open my $fh, '>', $creds_file or die "open $creds_file: $!";
print {$fh} '{"bad":"payload"}';
close $fh;
...
like($resp, qr/^HTTP\/1\.1 500 /m,
    'invalid credentials JSON in file returns 500');
like($resp, qr/\r\nContent-Length: 0\r\n/i,
