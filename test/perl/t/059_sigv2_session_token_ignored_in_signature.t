...
# SigV2 credential tests for ngx_http_s3_gateway_c_module.
# Focus: session token presence does not alter SigV2 signature value.
...
like($with_auth, qr/^AWS K:/, 'with-token request produces SigV2 header');
like($without_auth, qr/^AWS K:/, 'without-token request produces SigV2 header');
is($with_auth, $without_auth,
   'SigV2 signature is unchanged by session token presence');
