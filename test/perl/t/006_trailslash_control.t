...
# Trailing slash tests for ngx_http_s3_gateway_c_module.
# Cases are derived from njs s3gateway.js trailslashControl():
# - redirect only when APPEND_SLASH is enabled and path looks like a directory
...
like(http_get('/docs'), qr/^HTTP\/1\.1 302 /m, 'path without slash and extension redirects');
like(http_get('/docs/'), qr/^HTTP\/1\.1 404 /m, 'directory path with slash does not redirect');
like(http_get('/file.txt'), qr/^HTTP\/1\.1 404 /m, 'file path with extension does not redirect');
like(http_get('/with-query'), qr/^HTTP\/1\.1 302 /m, 'query/anchor are ignored for directory detection');
