...
# Focus: HEAD request follows same 404 probe fallback to @s3Directory as GET.
...
like($get_resp, qr/^HTTP\/1\.1 209 /m,
    'GET loadContent request falls back to @s3Directory on probe 404');
like($head_resp, qr/^HTTP\/1\.1 209 /m,
    'HEAD loadContent request also falls back to @s3Directory on probe 404');
unlike($head_resp, qr/directory$/s,
    'HEAD response keeps status fallback semantics without returning directory body');
