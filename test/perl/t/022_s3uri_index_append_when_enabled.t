...
# Focus: when PROVIDE_INDEX_PAGE=true and listing is disabled,
# directory paths must resolve to index.html in $s3uri.
...
like($dir, qr/^HTTP\/1\.1 200 /m, 'directory endpoint returns 200');
like($file, qr/^HTTP\/1\.1 200 /m, 'file endpoint returns 200');
like($dir, qr/\r\nX-S3URI: \/bucket-1\/docs\/index\.html\r\n/i,
    'directory URI resolves to index.html when PROVIDE_INDEX_PAGE=true');
like($file, qr/\r\nX-S3URI: \/bucket-1\/docs\/report\.txt\r\n/i,
    'file URI remains unchanged when PROVIDE_INDEX_PAGE=true');
