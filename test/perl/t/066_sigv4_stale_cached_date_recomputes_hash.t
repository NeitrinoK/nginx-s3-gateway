...
# Focus: stale cached date must not be reused; signing key is recomputed for current date.
...
my $yday = strftime("%Y%m%d", gmtime(time() - 86400));
my $stale_payload = join(',', (1) x 32);
...
http {
    ...
    server {
        ...
        location /stale-cache {
            set $s3_host bucket-1.s3.example.internal;
            set $cache_signing_key_enabled 1;
            set $signing_key_hash '__YDAY__:{"type":"Buffer","data":[__PAYLOAD__]}';
...
$conf =~ s/__YDAY__/$yday/g;
$conf =~ s/__PAYLOAD__/$stale_payload/g;
$t->write_file_expand('nginx.conf', $conf);

$t->try_run('no Test::Nginx runtime')->plan(4);

my $resp = http_get('/stale-cache');

like($resp, qr/^HTTP\/1\.1 200 /m, 'request returns 200');
like($resp, qr/\r\nX-S3AUTH: AWS4-HMAC-SHA256 Credential=AKIDEXAMPLE\//i,
    'SigV4 auth is generated');
unlike($resp, qr/\r\nX-SIGNING-KEY-HASH:\s*\Q$yday\E:\{"type":"Buffer","data":\[(?:1,){31}1\]\}\r\n/i,
    'stale-date cached value is not reused');
like($resp, qr/\r\nX-SIGNING-KEY-HASH:\s*\Q$today\E:\{"type":"Buffer","data":\[(?:\d{1,3},){31}\d{1,3}\]\}\r\n/i,
    'signing_key_hash is rebuilt for current date');
