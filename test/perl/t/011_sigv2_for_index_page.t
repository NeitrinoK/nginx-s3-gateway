...
# SigV2 tests for ngx_http_s3_gateway_c_module.
...
ok(defined $dir_auth && $dir_auth =~ /^AWS\s+AKIDEXAMPLE:/,
    'dir-sign produced non-empty SigV2 auth header');
ok(defined $file_auth && $file_auth =~ /^AWS\s+AKIDEXAMPLE:/,
    'file-sign produced non-empty SigV2 auth header');
is($dir_auth, $file_auth,
    'forIndexPage=true + directory path signs the same SigV2 target as explicit index.html path');

my ($dir_date) = ($dir_resp =~ /\r\nX-HTTPDATE:\s*([^\r\n]+)\r\n/i);
my ($file_date) = ($file_resp =~ /\r\nX-HTTPDATE:\s*([^\r\n]+)\r\n/i);
is($dir_date, $file_date, 'httpDate is identical for both requests in this module lifecycle');
