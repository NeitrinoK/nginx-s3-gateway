...
# Focus: GET directory uses listing query; HEAD directory signs object path.
...
my $head_resp = http_head('/dir');

like($get_resp, qr/^HTTP\/1\.1 200 /m, 'GET /dir returns 200');
like($head_resp, qr/^HTTP\/1\.1 200 /m, 'HEAD /dir returns 200');
like($get_resp, qr/\r\nX-S3URI: \/bucket-1\?delimiter=%2F&prefix=docs%2F\r\n/i,
    'GET /dir uses directory listing query parameters');
like($head_resp, qr/\r\nX-S3URI: \/bucket-1\/docs\/\r\n/i,
    'HEAD /dir keeps object-style URI without listing query');

my ($get_auth) = ($get_resp =~ /\r\nX-S3AUTH:\s*([^\r\n]+)\r\n/i);
my ($head_auth) = ($head_resp =~ /\r\nX-S3AUTH:\s*([^\r\n]+)\r\n/i);
ok(defined $get_auth && defined $head_auth, 'both GET and HEAD produce auth headers');
isnt($get_auth, $head_auth,
     'GET and HEAD directory requests produce different SigV4 signatures');
