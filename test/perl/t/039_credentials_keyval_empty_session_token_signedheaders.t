...
# Credential parsing tests for ngx_http_s3_gateway_c_module.
# Focus: empty sessionToken in keyval JSON is treated as no token for SigV4.
...
http {
    ...
    server {
        ...
        location / {
            ...
            set $instance_credential_json '{"accessKeyId":"KEYVALAK","secretAccessKey":"KEYVALSECRET","sessionToken":"","expiration":"2030-01-01T00:00:00Z"}';
...
like($resp, qr/\r\nX-TOKEN: \[\]\r\n/i,
    '$awsSessionToken is empty for empty keyval sessionToken');
like($resp, qr/\r\nX-S3AUTH: AWS4-HMAC-SHA256 Credential=KEYVALAK\//i,
    'SigV4 auth is still generated from keyval credentials');
like($resp, qr/SignedHeaders=host;x-amz-content-sha256;x-amz-date,Signature=/i,
    'SignedHeaders exclude x-amz-security-token for empty sessionToken');
unlike($resp, qr/x-amz-security-token/i,
      'auth header does not mention x-amz-security-token');
